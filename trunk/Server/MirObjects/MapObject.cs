...
namespace Server.MirObjects
{
    public abstract class MapObject
    {
        ...
        public int GetAttackPower(int min, int max)
        {
            ...
            int result = 0;
            int randValue = max - min;

            if (randValue < 0) randValue = 0;

            if (Luck > 0)
            {
                if (Envir.Random.Next(10 - Math.Min(9, (int)Luck)) == 0)
                {
                    result = min + randValue;
                }
                else
                {
                    result = min + Envir.Random.Next(randValue + 1);
                }
            }
            else
            {
                ...
                if (Luck < 0)
                {
                    if (Envir.Random.Next(Math.Max(0, 10 - Math.Max(0, -Luck))) == 0)
                    {
                        result = min;
                    }
                }
