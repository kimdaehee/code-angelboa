...
namespace Server.MirDatabase
{
    ...
    public class UserMagic
    {
        ...
        public byte getMaxTrainLevel(byte UserLevel)
        {
            if (UserLevel <= Info.Level1) return Info.Level1;
            if (UserLevel <= Info.Level2) return Info.Level2;
            if (UserLevel <= Info.Level3) return Info.Level3;
            //Check humUp
            //if (UserLevel <= Info.Level4) return Info.Level4;
        ...
        {
            return (int)Math.Round((10 + (Level / 3) * 0.9) * (power / 100));
        }

        public int Get5Power(int power)
        {
            return (int)Math.Round((50 + (Level / 3) * 0.5) * (power / 100));
        }

        public int MPow()
        {
            Random rand = new Random();
            return (int)(Info.MinPower + rand.Next(Info.MaxPower - Info.MinPower));
        }
        */
